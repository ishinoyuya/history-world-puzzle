"""
紀元前400年の世界地図(aourednik/historical-basemaps由来のGeoJSON、
ファイル名は world_bc400.geojson)を読み込み、経緯度(lon/lat)を
正距円筒図法でx,yに投影して、統合ページ用データ(../../data/bc400.json)
に変換する。

このデータセットは77件と中規模。実在した国家・王国だけを選ぶ
「採用リスト方式」にする。

紀元前400年はペロポネソス戦争終結(前404年)直後のギリシア古典期、
アケメネス朝ペルシアが最盛期。ローマはまだラティウム周辺に限られた
建国初期の小さな都市国家(周辺にはエトルリア・サビニ・サムニウムなど
中部イタリア諸民族が割拠)。インドはマガダ国がナンダ朝の下で強勢
(マウリヤ朝の建国=前321年頃はまだ先)。朝鮮半島は古朝鮮の時代。
中国は戦国時代(まだ秦は突出しておらず、他の戦国諸国とまとめて
データ化されている)。

「Zhou states」(秦を含む戦国諸国全体をまとめた曖昧な集合名で特定の
政体を指さない)、「Hindu kingdoms」(同様に曖昧な集合名)は不採用。
また「Celltic Hallsatt culture」(ハルシュタット文化)・「Late Jomon
culture」(縄文文化)・「Yayoi」(弥生)・「Olmec」「Zapotec」などの
考古学的な文化名は、特定の政体を意味しないため不採用とする
(Huari Empire・Carthaginian Empireのように「Empire/Kingdom」等の
政体を示す語を伴わない地名・文化名は一貫して不採用としている)。

「Greek city-states」はbbox確認の結果ギリシア本土(および周辺の
エーゲ海諸島)を指し、「Greek colonies」は南イタリア・シチリア、
コルシカ・サルデーニャ、黒海沿岸に点在する複数の植民市群を指す。
どちらも単一国家ではないが、他に代表する政体名が無いため地理的に
まとまった領域として採用する。

このデータセットには南米の帝国級エンティティや太平洋の政体が収録
されていないため、南北アメリカ・オセアニア両モードは0件で自動非表示
になる(紀元前300年以降と同様)。またNAME未設定のfeatureが非常に多数
...
# 実在した国家・王国のみを採用(77件中17件)。
INCLUDE_NAMES = {
    "Scythians", "Celts", "Greek city-states", "Greek colonies", "Boii",
    "Etrurians", "Sabini", "Samnites", "Roman Republic",
    "Carthaginian Empire", "Kush",
    "Achaemenid Empire", "Zhangzhung Kingdom", "Magadha", "Simhala",
    "Xiongnu", "Gojoseon",
}

NAME_JA = {
    "Scythians": "スキタイ", "Celts": "ケルト系諸部族(ブリテン諸島)",
    "Greek city-states": "ギリシア諸都市国家(本土)",
    "Greek colonies": "ギリシア植民市(地中海各地)",
    "Boii": "ボイイ族(ケルト系)", "Etrurians": "エトルリア(諸都市国家)",
    "Sabini": "サビニ族", "Samnites": "サムニウム人",
    "Roman Republic": "ローマ共和国(建国初期)",
    "Carthaginian Empire": "カルタゴ", "Kush": "クシュ王国(ナパタ王国)",
    "Achaemenid Empire": "アケメネス朝ペルシア",
    "Zhangzhung Kingdom": "シャンシュン王国", "Magadha": "マガダ国(ナンダ朝)",
    "Simhala": "シンハラ王国(アヌラーダプラ)",
    "Xiongnu": "匈奴", "Gojoseon": "古朝鮮",
}
...
LABEL_JA_SHORT = {
    "Greek city-states": "ギリシア諸都市国家", "Greek colonies": "ギリシア植民市",
    "Etrurians": "エトルリア", "Kush": "クシュ王国",
...
def main():
    src = json.load(open(os.path.join(BASE, "world_bc400.geojson"), encoding="utf-8"))
    feats = src["features"]
    ...
    # 統合ページ(world-history-puzzle)の data/bc400.json に直接出力する
    out_dir = os.path.join(BASE, "..", "..", "data")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "bc400.json")
