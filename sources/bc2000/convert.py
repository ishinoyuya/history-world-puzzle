"""
紀元前2000年の世界地図(aourednik/historical-basemaps由来のGeoJSON、
ファイル名は world_bc2000.geojson)を読み込み、経緯度(lon/lat)を
正距円筒図法でx,yに投影して、統合ページ用データ(../../data/bc2000.json)
に変換する。

このデータセットは46件と小規模で、青銅器時代の初期文明が中心となる。
実在した国家・文明だけを選ぶ「採用リスト方式」にする。

紀元前2000年はウル第三王朝が崩壊した直後(前2004年頃)の混乱期、
エジプトは中王国時代、インダス文明が最盛期、ミノア文明(クレタ)が
古宮殿期の始まり。中国は夏王朝(伝承上は前2070年頃建国とされ、考古学
的にはこの時期の二里頭文化に相当するとされるが史実性には議論がある)。
中央アジアはオクサス文明(バクトリア・マルギアナ複合、都市化した
青銅器文明)。

「Hittites」表記の領域は、実際のヒッタイト王国の建国(前1650年頃)より
350年ほど早く、この時代のアナトリアにはまだヒッタイト王国が存在しない
ため不採用とする。「Hurrian Kingdoms」(複数の政体をまとめた曖昧な
集合名)、「city-states」(北メソポタミアの諸都市国家を指すと思われる
が具体名が無い曖昧な集合名)も不採用。「Namazga」は考古学的な文化圏
区分名であり、オクサス文明ほど都市化した政治体制を示す語ではないため
不採用とする一方、「Oxus」は都市遺跡を伴う複合文明として広く認知されて
いるため採用する。また「Semites」「Bantu」「Dravidians」「Koreans」等の
民族・言語集団名、および考古学的な文化名(Beaker、Andronovo、Sintashta、
Jōmon等)も特定の政体を意味しないため不採用とする。

このデータセットには南米の帝国級エンティティ(Huari・Tiahuanacoに相当
する規模の政体)や太平洋の政体、朝鮮半島の具体的な政体が収録されて
いないため、南北アメリカ・オセアニアが自動非表示になる(東アジアは
夏のみのため1件で表示される)。またNAME未設定のfeatureが非常に多数
あり、操作不可の背景landとして描画する。
...
# 実在した国家・文明のみを採用(46件中10件)。
INCLUDE_NAMES = {
    "Minoan", "Cycladic",
    "Egypt", "Kerma",
    "Ur", "Elam", "Canaan", "Oxus", "Indus valley civilization",
    "Xia",
}

NAME_JA = {
    "Minoan": "ミノア文明(クレタ)", "Cycladic": "キクラデス文明",
    "Egypt": "エジプト(中王国時代)", "Kerma": "ケルマ王国(ヌビア)",
    "Ur": "ウル(シュメール、ウル第三王朝末期)", "Elam": "エラム王国",
    "Canaan": "カナン(諸都市国家)",
    "Oxus": "オクサス文明(バクトリア・マルギアナ複合)",
...
LABEL_JA_SHORT = {
    "Minoan": "ミノア文明", "Kerma": "ケルマ王国", "Ur": "ウル(シュメール)",
    "Canaan": "カナン諸都市国家", "Oxus": "オクサス文明", "Xia": "夏王朝",
}
...
def main():
    src = json.load(open(os.path.join(BASE, "world_bc2000.geojson"), encoding="utf-8"))
    feats = src["features"]
    ...
    # 統合ページ(world-history-puzzle)の data/bc2000.json に直接出力する
    out_dir = os.path.join(BASE, "..", "..", "data")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "bc2000.json")
