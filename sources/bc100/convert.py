"""
紀元前100年の世界地図(aourednik/historical-basemaps由来のGeoJSON、
ファイル名は world_bc100.geojson)を読み込み、経緯度(lon/lat)を
正距円筒図法でx,yに投影して、統合ページ用データ(../../data/bc100.json)
に変換する。

このデータセットは85件と中規模。実在した国家・王国だけを選ぶ
「採用リスト方式」にする。

紀元前100年はローマがまだ共和政期(帝政開始は前27年)。プトレマイオス朝
エジプト・セレウコス朝(衰退期、滅亡は前63年)・パルティア帝国・前漢が
それぞれ健在。南アラビアはサバア・カタバーン・マイーンなど複数の
香料交易王国が並立。中央アジアはサカ族(スキタイ系遊牧民)がバクトリア・
ソグディアナ方面へ移動・侵入している時期。

「Mauryan Empire」表記の領域は、マウリヤ朝自体は前185年に滅亡済みだが、
bboxがガンジス川流域を中心とするインド亜大陸の広い範囲と一致することから、
実効支配の連続性に基づき後継のシュンガ朝として読み替えて採用する。

年代的に不適切と判断し不採用としたもの:
- Kushan Empire(クシャーナ朝の建国は後30年頃で130年ほど早すぎる)
- Nan-Yue(南越は前111年に前漢に征服済み)
- Min-Yue(閩越は前110年に前漢に征服済み)
- minor states(特定の政体を指さない曖昧な集合名)
- Hövsgöl Nuur(モンゴルのフブスグル湖を指す地名で、政体ではない)

このデータセットには南米の帝国級エンティティや太平洋の政体、日本列島の
政体が収録されていないため、南北アメリカ・オセアニア両モードは0件で
自動非表示になる(紀元前1年以降と同様)。またNAME未設定のfeatureが非常に
...
# 実在した国家・王国のみを採用(85件中24件)。
INCLUDE_NAMES = {
    "Roman Republic", "Celts", "Daces", "Bosporan Kingdom", "Sarmates",
    "Ptolemaic Kingdom", "Meroe", "Numidia", "Mauretania",
    "Mauryan Empire", "Seleucid Kingdom", "Zhangzhung Kingdom", "Parthia",
    "Qataban", "Maan", "Nabatean Kingdom", "Hadramaut", "Saba", "Simhala",
    "Saces", "Scythians",
    "Xiongnu", "Han Empire", "Arakan",
}

NAME_JA = {
    "Roman Republic": "ローマ共和国", "Celts": "ケルト系諸部族(ブリテン諸島)",
    "Daces": "ダキア人(統一前の諸部族)", "Bosporan Kingdom": "ボスポロス王国",
    "Sarmates": "サルマタイ",
    "Ptolemaic Kingdom": "プトレマイオス朝エジプト",
    "Meroe": "メロエ王国(クシュ王国)", "Numidia": "ヌミディア王国",
    "Mauretania": "マウレタニア王国",
    "Mauryan Empire": "シュンガ朝", "Seleucid Kingdom": "セレウコス朝(衰退期)",
    "Zhangzhung Kingdom": "シャンシュン王国", "Parthia": "パルティア帝国",
    "Qataban": "カタバーン王国", "Maan": "マイーン王国",
    "Nabatean Kingdom": "ナバテア王国", "Hadramaut": "ハドラマウト王国",
    "Saba": "サバア王国(シバの女王の国)",
    "Simhala": "シンハラ王国(アヌラーダプラ)",
    "Saces": "サカ族(ソグディアナ方面)",
    "Scythians": "サカ・スキタイ(中央アジア方面)",
    "Xiongnu": "匈奴", "Han Empire": "前漢",
...
REGIONS = {
    ...
    "wsasia": [
        "Mauryan Empire", "Seleucid Kingdom", "Zhangzhung Kingdom",
        "Parthia", "Qataban", "Maan", "Nabatean Kingdom", "Hadramaut",
        "Saba", "Simhala", "Saces", "Scythians",
    ],
    "easia": [
        "Xiongnu", "Han Empire", "Arakan",
    ],
...
LABEL_JA_SHORT = {
    "Celts": "ケルト系諸部族", "Daces": "ダキア人",
    "Ptolemaic Kingdom": "プトレマイオス朝", "Seleucid Kingdom": "セレウコス朝",
    "Saba": "サバア王国", "Saces": "サカ族", "Arakan": "アラカン王国",
}
...
def main():
    src = json.load(open(os.path.join(BASE, "world_bc100.geojson"), encoding="utf-8"))
    feats = src["features"]
    ...
    # 統合ページ(world-history-puzzle)の data/bc100.json に直接出力する
    out_dir = os.path.join(BASE, "..", "..", "data")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "bc100.json")
