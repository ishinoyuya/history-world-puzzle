"""
紀元前700年の世界地図(aourednik/historical-basemaps由来のGeoJSON、
ファイル名は world_bc700.geojson)を読み込み、経緯度(lon/lat)を
正距円筒図法でx,yに投影して、統合ページ用データ(../../data/bc700.json)
に変換する。

このデータセットは74件と中規模。実在した国家・王国だけを選ぶ
「採用リスト方式」にする。

紀元前700年は新アッシリア帝国が最盛期に向かう時代。エジプトは末期
王朝時代(クシュ王国出身の第25王朝の治世)。「Kingdom of David and
Solomon」表記の領域は、統一王国(ダビデ・ソロモン王、前10世紀)は
既に南北分裂しており、さらに北のイスラエル王国は前722年にアッシリアに
滅ぼされている。bboxが確認できる範囲は生き残った南のユダ王国の版図と
一致するため、実効支配の連続性に基づき「ユダ王国」(ヒゼキヤ王の治世、
アッシリアによるエルサレム包囲は前701年)として読み替えて採用する。
中国は春秋時代で、呉のみ個別に識別できる政体として採用する
(「Zhoa」「Sinic」はどちらも特定の政体を指さない曖昧な集合名)。

「Hittites」表記の領域は、ヒッタイト帝国自体は前1200年頃に滅亡済みで、
後継のネオ・ヒッタイト諸国もこの時代までにほぼアッシリアへ併合されて
おり、bboxもアッシリア領と重複するため不採用とする。「Arameans」
「state societies and Aramaean kingdoms」(曖昧な集合名・民族名で
特定の政体を指さない)も同様に不採用。また「Vedic Aryans」
「Dravidians」「Tibetans」「Burmese」「Berbers」等の民族・言語集団名、
および考古学的な文化名(Karasuk culture、La Tène culture等)も政体を
意味しないため不採用とする。

このデータセットには南米の帝国級エンティティや太平洋の政体、朝鮮半島・
日本列島の政体(いずれも曖昧なProto-/Paleo-系の民族ラベルのみで具体的
政体名が無い)も収録されていないため、南北アメリカ・オセアニアに加えて
東アジア・東南アジアモードもほぼ0件近くまで縮小する(呉のみ採用の
ため0件にはならないが1件のみ)。またNAME未設定のfeatureが非常に多数
...
# 実在した国家・王国のみを採用(74件中15件)。
INCLUDE_NAMES = {
    "Thrace", "Phrygians", "Cimerians", "Illyrians", "Celtiberians",
    "Greek city-states",
    "Egypt", "Kush",
    "Assyria", "Urartu", "Elam", "Babylonia",
    "Kingdom of David and Solomon", "Saba",
    "Wu",
}

NAME_JA = {
    "Thrace": "トラキア", "Phrygians": "フリギア王国",
    "Cimerians": "キンメリア人(遊牧民)", "Illyrians": "イリュリア人(諸部族)",
    "Celtiberians": "ケルティベリア人", "Greek city-states": "ギリシア諸都市国家",
    "Egypt": "エジプト(末期王朝時代)", "Kush": "クシュ王国(第25王朝)",
    "Assyria": "新アッシリア帝国", "Urartu": "ウラルトゥ王国",
    "Elam": "エラム王国", "Babylonia": "バビロニア(アッシリア支配下)",
    "Kingdom of David and Solomon": "ユダ王国(ヒゼキヤ王)",
    "Saba": "サバア王国(シバの女王の国)", "Wu": "呉(春秋時代)",
}

# 地域モード用の区分。このデータセットには南米の帝国級エンティティ・
...
LABEL_JA_SHORT = {
    "Cimerians": "キンメリア人", "Illyrians": "イリュリア人",
    "Egypt": "エジプト", "Kush": "クシュ王国", "Babylonia": "バビロニア",
    "Kingdom of David and Solomon": "ユダ王国", "Saba": "サバア王国",
}
...
def main():
    src = json.load(open(os.path.join(BASE, "world_bc700.geojson"), encoding="utf-8"))
    feats = src["features"]
    ...
    # 統合ページ(world-history-puzzle)の data/bc700.json に直接出力する
    out_dir = os.path.join(BASE, "..", "..", "data")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "bc700.json")
