"""
紀元前300年の世界地図(aourednik/historical-basemaps由来のGeoJSON、
ファイル名は world_bc300.geojson)を読み込み、経緯度(lon/lat)を
正距円筒図法でx,yに投影して、統合ページ用データ(../../data/bc300.json)
に変換する。

このデータセットは74件と中規模。実在した国家・王国だけを選ぶ
「採用リスト方式」にする。

紀元前300年はアレクサンドロス大王死後(前323年)のディアドコイ戦争
(後継者戦争)の時代。イプソスの戦い(前301年)でアンティゴノス1世が
戦死した直後で、その版図はリュシマコス・セレウコス朝・カッサンドロス
らに再分割されている。マウリヤ朝はチャンドラグプタ王による建国期。
中国は戦国時代で秦が台頭中。ローマはまだ中部イタリアに限られた
共和政初期(サムニウム戦争期)。カルタゴは最盛期に近い。

「Kingdom of Antigonus」表記の領域は、アンティゴノス1世自身は前301年の
イプソスの戦いで戦死しその王国は解体済みであり、同じ領域を「Kingdom
of Lysimachus」「Seleucid Kingdom」など他の後継者国家が別途採用済みの
ため、重複を避けるべく不採用とする。「Zhow states」(秦以外の戦国
諸国をまとめた曖昧な集合名)、「Hindu kingdoms and republics」「Hindu
kingdoms」(いずれも特定の政体を指さない曖昧な集合名)も同様の理由で
不採用。

「Greek city-states」表記の領域はbbox確認の結果、ギリシア本土ではなく
南イタリア・シチリア(マグナ・グラエキア)のギリシア植民市群を指して
いる。単一の国家ではなく複数の独立ポリスの集合だが、他に代表する
政体名が無く空白地帯になってしまうため、地理的にまとまった一領域として
採用する。

このデータセットには南米の帝国級エンティティや太平洋の政体、日本列島の
政体が収録されていないため、南北アメリカ・オセアニア両モードは0件で
自動非表示になる(紀元前200年以降と同様)。またNAME未設定のfeatureが
非常に多数(南極大陸・サハラ砂漠内陸部・シベリア内陸部・グリーンランド
など無所属の広大な陸地)あり、操作不可の背景landとして描画する。
...
# 実在した国家・王国のみを採用(74件中26件)。
INCLUDE_NAMES = {
    "Celts", "Cappadocia", "Kingdom of Lysimachus", "Kingdom of Kassander",
    "Armenia", "Bosporan Kingdom", "Greek city-states", "Colchis",
    "Roman Republic", "Crete", "Bithynia",
    "Carthaginian Empire", "Ptolemaic Kingdom", "Meroe", "Damot",
    "Seleucid Kingdom", "Mauryan Empire", "Zhangzhung Kingdom", "Qataban",
    "Atropatene", "Nabatean Kingdom", "Hadramaut", "Saba", "Simhala",
    "Qin", "Yue",
}

NAME_JA = {
    "Celts": "ケルト系諸部族(ブリテン諸島)", "Cappadocia": "カッパドキア王国",
    "Kingdom of Lysimachus": "リュシマコス王国(トラキア)",
    "Kingdom of Kassander": "カッサンドロス王国(マケドニア)",
    "Armenia": "アルメニア王国", "Bosporan Kingdom": "ボスポロス王国",
    "Greek city-states": "ギリシア植民市(マグナ・グラエキア)",
    "Colchis": "コルキス王国", "Roman Republic": "ローマ共和国",
    "Crete": "クレタ島(諸都市国家)", "Bithynia": "ビテュニア王国",
    "Carthaginian Empire": "カルタゴ",
    "Ptolemaic Kingdom": "プトレマイオス朝エジプト",
    "Meroe": "メロエ王国(クシュ王国)", "Damot": "ダモト王国(エチオピア高原)",
    "Seleucid Kingdom": "セレウコス朝(建国期)",
    "Mauryan Empire": "マウリヤ朝(建国期)",
    "Zhangzhung Kingdom": "シャンシュン王国", "Qataban": "カタバーン王国",
    "Atropatene": "アトロパテネ王国", "Nabatean Kingdom": "ナバテア王国",
    "Hadramaut": "ハドラマウト王国", "Saba": "サバア王国(シバの女王の国)",
    "Simhala": "シンハラ王国(アヌラーダプラ)",
    "Qin": "秦(戦国時代)", "Yue": "越(滅亡後の南方諸族)",
}
...
REGIONS = {
    ...
    "wsasia": [
        "Seleucid Kingdom", "Mauryan Empire", "Zhangzhung Kingdom",
        "Qataban", "Atropatene", "Nabatean Kingdom", "Hadramaut", "Saba",
        "Simhala",
    ],
    "easia": [
        "Qin", "Yue",
...
LABEL_JA_SHORT = {
    "Kingdom of Lysimachus": "リュシマコス王国",
    "Kingdom of Kassander": "カッサンドロス王国",
    "Greek city-states": "ギリシア植民市", "Ptolemaic Kingdom": "プトレマイオス朝",
    "Damot": "ダモト王国", "Seleucid Kingdom": "セレウコス朝",
    "Mauryan Empire": "マウリヤ朝", "Saba": "サバア王国",
    "Celts": "ケルト系諸部族", "Yue": "越",
}
...
def main():
    src = json.load(open(os.path.join(BASE, "world_bc300.geojson"), encoding="utf-8"))
    feats = src["features"]
    ...
    # 統合ページ(world-history-puzzle)の data/bc300.json に直接出力する
    out_dir = os.path.join(BASE, "..", "..", "data")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "bc300.json")
