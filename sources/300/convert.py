"""
300年の世界地図(aourednik/historical-basemaps由来のGeoJSON)を読み込み、
経緯度(lon/lat)を正距円筒図法でx,yに投影して、統合ページ用データ
(../../data/300.json)に変換する。

このデータセットは72件と中規模。実在した国家・王国だけを選ぶ
「採用リスト方式」にする。

300年はディオクレティアヌス帝によるテトラルキア(四帝統治制、293年
導入)の時代で、ローマ帝国は正帝2名(西方=マクシミアヌス、東方=
ディオクレティアヌス)・副帝2名(西方=コンスタンティウス、東方=
ガレリウス)による分割統治下にあり、データセットもこの4分割を
忠実に反映しているためそのまま4ピースとして採用する。中国は
まだ西晋によって統一されている(五胡十六国の混乱が始まるのは
304年以降)。

「Parthian Empire」表記の領域は、パルティア帝国自体は224年に
サーサーン朝に取って代わられ滅亡済みだが、bboxがペルシア・
メソポタミア地域と一致することから実効支配の連続性に基づき
「サーサーン朝ペルシア」として読み替えて採用する。同様にデカン
高原の「Satavahanihara」もヴァーカータカ朝(建国は250年頃)として
読み替えて採用。

年代的に不適切と判断し不採用としたもの:
- Gupta Empire(グプタ朝の建国は320年で20年早い)
- Han Zhao(漢趙の建国は304年で4年早く、300年時点では西晋領のまま)
- Dumonii(ブリテン島南西部の部族名だが、300年時点ではまだローマ帝国の
  属州民でありローマ帝国の領域と重複する)

このデータセットには南米の帝国級エンティティや太平洋の政体が収録
されていないため、南北アメリカ・オセアニア両モードは0件で自動非表示
になる(400年・500年と同様)。またNAME未設定のfeatureが非常に多数
...
# 実在した国家・王国のみを採用(72件中23件)。
INCLUDE_NAMES = {
    "Rome (Maximian)", "Rome (Diocletianus)", "Rome (Constantinus)",
    "Rome (Galerius)", "Armenia", "Heruli",
    "Meroe", "Empire of Ghana", "Axum",
    "Parthian Empire", "Saka Kingdom", "Suren Kingdom", "Satavahanihara",
    "Himyarite Kingdom", "Hadramaut", "Kalinga", "Simhala",
    "Jin", "Koguryo", "Yueban", "Silla", "Paekche", "Gaya",
}

NAME_JA = {
    "Rome (Maximian)": "ローマ帝国(西方正帝マクシミアヌス)",
    "Rome (Diocletianus)": "ローマ帝国(東方正帝ディオクレティアヌス)",
    "Rome (Constantinus)": "ローマ帝国(西方副帝コンスタンティウス)",
    "Rome (Galerius)": "ローマ帝国(東方副帝ガレリウス)",
    "Armenia": "アルメニア王国", "Heruli": "ヘルリ族",
    "Meroe": "メロエ王国(クシュ王国)", "Empire of Ghana": "ガーナ帝国",
    "Axum": "アクスム王国",
    "Parthian Empire": "サーサーン朝ペルシア", "Saka Kingdom": "西クシャトラパ朝(サカ族)",
    "Suren Kingdom": "スレーン家領(シースターン)",
    "Satavahanihara": "ヴァーカータカ朝(建国期)",
    "Himyarite Kingdom": "ヒムヤル王国(イエメン)", "Hadramaut": "ハドラマウト王国",
    "Kalinga": "カリンガ王国", "Simhala": "シンハラ王国(アヌラーダプラ)",
    "Jin": "西晋", "Koguryo": "高句麗", "Yueban": "悦般(北匈奴残存勢力)",
    "Silla": "新羅(建国期、斯盧国)", "Paekche": "百済(建国期)",
    "Gaya": "加耶諸国",
}
...
LABEL_JA_SHORT = {
    "Rome (Maximian)": "ローマ帝国(マクシミアヌス)",
    "Rome (Diocletianus)": "ローマ帝国(ディオクレティアヌス)",
    "Rome (Constantinus)": "ローマ帝国(コンスタンティウス)",
    "Rome (Galerius)": "ローマ帝国(ガレリウス)",
    "Meroe": "メロエ王国", "Saka Kingdom": "西クシャトラパ朝",
    "Suren Kingdom": "スレーン家領", "Satavahanihara": "ヴァーカータカ朝",
    "Himyarite Kingdom": "ヒムヤル王国", "Silla": "新羅(建国期)",
    "Paekche": "百済(建国期)", "Yueban": "悦般",
}
...
def main():
    src = json.load(open(os.path.join(BASE, "world_300.geojson"), encoding="utf-8"))
    feats = src["features"]
    ...
    # 統合ページ(world-history-puzzle)の data/300.json に直接出力する
    out_dir = os.path.join(BASE, "..", "..", "data")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "300.json")
