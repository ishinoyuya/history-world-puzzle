"""
600年の世界地図(aourednik/historical-basemaps由来のGeoJSON)を読み込み、
経緯度(lon/lat)を正距円筒図法でx,yに投影して、統合ページ用データ
(../../data/600.json)に変換する。

このデータセットは84件と中規模。実在した国家・王国・首長国だけを選ぶ
「採用リスト方式」にする。

600年は隋の中国統一後(隋は700年データセットでは唐と読み替えたが、
今回は隋自体が実在する年代なのでそのまま「隋」として採用)、
サーサーン朝ペルシアがホスロー2世の下で最盛期、東ローマ帝国(ビザンツ)、
高句麗はまだ健在(滅亡は668年)、百済もまだ健在(滅亡は660年)。
イベリア半島は西ゴート王国領。フランク王国はメロヴィング朝でネウストリア
などの副王国を伴う。

一方、以下は年代的に不適切と判断し不採用とした:
- Khazars(ハザール建国は7世紀半ば以降、600年時点では独立国家未形成)
- Bulgars(統一ブルガール国家の成立は632年のクブラト以降)
- Ostrogoths(東ゴート王国は553年の東ローマとの戦争で滅亡済み)
- Mataram(マタラム王国の建国は732年)
- Tuʻi Tonga Empire(トゥイ・トンガ王朝の伝統的建国年は950年頃で、
  600年時点では約350年早い。このデータセットには他に太平洋の政体は
  存在しないため、オセアニアモードは0件で自動非表示になる)

なお、このデータセットには「NAME未設定」のfeatureが多数(南極大陸・
サハラ砂漠内陸部・シベリア内陸部・グリーンランドなど無所属の広大な陸地)
あり、700年・800年と同様に操作不可の背景landとして描画する。
...
# 実在した国家・王国・首長国のみを採用(84件中41件)。
INCLUDE_NAMES = {
    "Eastern Roman Empire", "Frankish Kingdom", "Neustria",
    "Visigothic Kingdom", "Avars", "Lombard principalities", "Cantia",
    "Saxons", "Frisians", "Swedes", "Danes", "Dumnonia",
    "Empire of Ghana", "Makkura", "Alwa", "Nobatia", "Axum",
    "Tiahuanaco Empire", "Huari Empire",
    "Sasanian Empire", "Sasanian dependencies", "Kanauj", "Chalukyas",
    "Shaskanka", "Pallavas", "Hejaz", "Kamarupa", "Chauhans", "Mazun",
    "Simhala",
    "Göktürks", "Sui Empire", "Koguryo", "Yamato", "Chen-La", "Dvaravati",
    "Funan", "Champa", "Silla", "Paekche", "Arakan",
}

NAME_JA = {
    "Eastern Roman Empire": "東ローマ帝国(ビザンツ帝国)",
    "Frankish Kingdom": "フランク王国(メロヴィング朝)",
    "Neustria": "ネウストリア(西フランク)", "Visigothic Kingdom": "西ゴート王国",
    "Avars": "アヴァール可汗国", "Lombard principalities": "ランゴバルド王国・南イタリア諸公国",
    "Cantia": "ケント王国", "Saxons": "ザクセン人(独立部族連合)",
    "Frisians": "フリース人(フリースラント)", "Swedes": "スウェーデン(スヴェア人)",
    "Danes": "デンマーク(デーン人)", "Dumnonia": "ドゥムノニア王国(コーンウォール)",
    "Empire of Ghana": "ガーナ帝国", "Makkura": "マクリア王国(ヌビア)",
    "Alwa": "アルワ王国", "Nobatia": "ノバティア王国(ヌビア)", "Axum": "アクスム王国",
    "Tiahuanaco Empire": "ティワナク帝国", "Huari Empire": "ワリ帝国",
    "Sasanian Empire": "サーサーン朝ペルシア", "Sasanian dependencies": "サーサーン朝の従属領",
    "Kanauj": "カナウジ(タネーサル王国)", "Chalukyas": "チャールキヤ朝",
    "Shaskanka": "ガウダ王国(シャシャーンカ)", "Pallavas": "パッラヴァ朝",
    "Hejaz": "ヒジャーズ(イスラーム以前のアラビア)",
    "Kamarupa": "カーマルーパ王国(アッサム)",
    "Chauhans": "チャハマーナ朝(チャウハーン朝)", "Mazun": "マズーン(オマーン)",
    "Simhala": "シンハラ王国(アヌラーダプラ)",
    "Göktürks": "突厥(第一可汗国)", "Sui Empire": "隋", "Koguryo": "高句麗",
    "Yamato": "日本(大和・飛鳥時代)", "Chen-La": "チェンラ王国(真臘)",
    "Dvaravati": "ドヴァーラヴァティー王国", "Funan": "扶南",
    "Champa": "チャンパ王国", "Silla": "新羅", "Paekche": "百済",
    "Arakan": "アラカン王国(ヴェーサリー朝)",
}

# 地域モード用の区分。このデータセットには実在する太平洋の政体が存在しない
# ため(Tuʻi Tonga Empireは年代的に不採用)oceaniaキーは意図的に用意して
# いない(0件のモードボタンはゲーム側で自動非表示)。
REGIONS = {
    "europe": [
        "Eastern Roman Empire", "Frankish Kingdom", "Neustria",
        "Visigothic Kingdom", "Avars", "Lombard principalities", "Cantia",
    ...
    "easia": [
        "Göktürks", "Sui Empire", "Koguryo", "Yamato", "Chen-La",
        "Dvaravati", "Funan", "Champa", "Silla", "Paekche", "Arakan",
    ],
}
COUNTRY_REGIONS = {}
...
LABEL_JA_SHORT = {
    ...
    "Lombard principalities": "ランゴバルド諸公国", "Dumnonia": "ドゥムノニア王国",
    "Sasanian Empire": "サーサーン朝", "Sasanian dependencies": "サーサーン朝従属領",
    "Kanauj": "カナウジ", "Shaskanka": "ガウダ王国", "Hejaz": "ヒジャーズ",
    "Kamarupa": "カーマルーパ王国", "Chauhans": "チャウハーン朝",
    "Yamato": "日本(飛鳥時代)", "Chen-La": "チェンラ王国",
    "Dvaravati": "ドヴァーラヴァティー", "Arakan": "アラカン王国",
}
...
def main():
    src = json.load(open(os.path.join(BASE, "world_600.geojson"), encoding="utf-8"))
    feats = src["features"]
    ...
    # 統合ページ(world-history-puzzle)の data/600.json に直接出力する
    out_dir = os.path.join(BASE, "..", "..", "data")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "600.json")
