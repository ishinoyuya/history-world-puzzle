"""
紀元前200年の世界地図(aourednik/historical-basemaps由来のGeoJSON、
ファイル名は world_bc200.geojson)を読み込み、経緯度(lon/lat)を
正距円筒図法でx,yに投影して、統合ページ用データ(../../data/bc200.json)
に変換する。

このデータセットは87件と中規模。実在した国家・王国だけを選ぶ
「採用リスト方式」にする。

紀元前200年は第二次ポエニ戦争終結(前201年)直後で、ローマは共和政期。
カルタゴはまだ滅亡していない(滅亡は前146年)。マウリヤ朝も健在
(滅亡は前185年)。マケドニア王国はヘラス同盟を率い第二次マケドニア
戦争(前200年開戦)の直前。南越・閩越はどちらも建国間もない(南越の
建国は前204年)。バクトリアはグレコ・バクトリア王国として最盛期に
近づいている。ヌミディアはシファクス(西)とガラ(東)の2王家に分裂
していた最末期(この2年後にはマシニッサによる統一が進む)。

「?」という名前(バルカン半島)は元データが特定の政体名を判別できな
かった箇所を示すプレースホルダーであり、意味のあるラベルを提供でき
ないため不採用とする。

このデータセットには南米の帝国級エンティティや太平洋の政体、日本列島の
政体が収録されていないため、南北アメリカ・オセアニア両モードは0件で
自動非表示になる(紀元前100年以降と同様)。またNAME未設定のfeatureが
...
# 実在した国家・王国のみを採用(87件中30件)。
INCLUDE_NAMES = {
    "Scythians", "Rome", "Celts", "Macedon and Hellenic League",
    "Bosporan Kingdom", "Dardania", "Pergamon",
    "Ptolemaic Kingdom", "Meroe", "Kingdom of Syphax", "Mauretania",
    "Carthage", "Kingdom of Gala",
    "Mauryan Empire", "Seleucid Kingdom", "Bactria", "Parthia",
    "Zhangzhung Kingdom", "Qataban", "Maan", "Nabatean Kingdom",
    "Hadramaut", "Saba", "Simhala",
    "Xiongnu", "Yuezhi", "Han Empire", "Min-Yue", "Nan-Yue", "Arakan",
}

NAME_JA = {
    "Scythians": "スキタイ", "Rome": "ローマ共和国",
    "Celts": "ケルト系諸部族(ブリテン諸島)",
    "Macedon and Hellenic League": "マケドニア王国とヘラス同盟",
    "Bosporan Kingdom": "ボスポロス王国", "Dardania": "ダルダニア王国",
    "Pergamon": "ペルガモン王国",
    "Ptolemaic Kingdom": "プトレマイオス朝エジプト",
    "Meroe": "メロエ王国(クシュ王国)",
    "Kingdom of Syphax": "シファクス王国(西ヌミディア)",
    "Mauretania": "マウレタニア王国", "Carthage": "カルタゴ",
    "Kingdom of Gala": "ガラ王国(東ヌミディア)",
    "Mauryan Empire": "マウリヤ朝", "Seleucid Kingdom": "セレウコス朝",
    "Bactria": "グレコ・バクトリア王国", "Parthia": "パルティア帝国(建国期)",
    "Zhangzhung Kingdom": "シャンシュン王国", "Qataban": "カタバーン王国",
    "Maan": "マイーン王国", "Nabatean Kingdom": "ナバテア王国",
    "Hadramaut": "ハドラマウト王国", "Saba": "サバア王国(シバの女王の国)",
    "Simhala": "シンハラ王国(アヌラーダプラ)",
    "Xiongnu": "匈奴", "Yuezhi": "月氏", "Han Empire": "前漢",
    "Min-Yue": "閩越", "Nan-Yue": "南越",
...
LABEL_JA_SHORT = {
    "Macedon and Hellenic League": "マケドニア・ヘラス同盟",
    "Ptolemaic Kingdom": "プトレマイオス朝", "Kingdom of Syphax": "シファクス王国",
    "Kingdom of Gala": "ガラ王国", "Bactria": "グレコ・バクトリア",
    "Parthia": "パルティア帝国", "Saba": "サバア王国",
    "Celts": "ケルト系諸部族", "Arakan": "アラカン王国",
}
...
def main():
    src = json.load(open(os.path.join(BASE, "world_bc200.geojson"), encoding="utf-8"))
    feats = src["features"]
    ...
    # 統合ページ(world-history-puzzle)の data/bc200.json に直接出力する
    out_dir = os.path.join(BASE, "..", "..", "data")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "bc200.json")
