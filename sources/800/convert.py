"""
800年の世界地図(aourednik/historical-basemaps由来のGeoJSON)を読み込み、
経緯度(lon/lat)を正距円筒図法でx,yに投影して、統合ページ用データ
(../../data/800.json)に変換する。

このデータセットは118件と小規模。実在した国家・帝国・王国だけを選ぶ
「採用リスト方式」にする。

800年はカール大帝が西ローマ皇帝として戴冠した年(まだフランク王国は
東西分裂前の統一カロリング帝国)。アッバース朝はハールーン・アッラシード
の治世で最盛期。唐は徳宗の治世。ビザンツ帝国も健在。イングランドは
ヘプターキー(七王国)期でマーシア・ウェセックス・ノーサンブリア・
エセックス・ケントが残存。インドはラーシュトラクータ朝・グルジャラ・
プラティーハーラ朝・パーラ朝の「三国時代」。日本は「日本(平安時代)」
表記。クメール帝国はまだ成立前(802年建国)のためチェンラ王国のまま。
なお、このデータセットには太平洋の政体が一切収録されていないため、
オセアニアの地域モードは0件になる(ゲーム側で0件モードは自動的に
非表示にする対応済み)。

このデータセットには「NAME未設定」のfeatureが103件あり、南極大陸・
サハラ砂漠内陸部・シベリア内陸部・グリーンランドなど、特定の政体に
属さない広大な陸地を表す。これらは国名を持たないため採用リストの
対象にはならないが、操作不可の背景land(neutral)として描画しないと
地図上に大きな空白ができてしまうため、excluded_polygonsに含める。
...
# 実在した国家・帝国・王国のみを採用(118件中57件)。
INCLUDE_NAMES = {
    "Byzantine Empire", "Carolingian Empire", "Khazars", "Volga Bulgars",
    "Danes", "Swedes and Goths", "Bulgars", "Asturias", "Emirate of Córdoba",
    "Cyprus", "Britany", "Irlanda", "Welsh", "Scots", "Picts", "Mercia",
    "Wessex", "Nothumbria", "Essex", "Kent", "Lombard duchies", "Magyars",
    "Rus' Khaganate", "Papal States", "Avars",
    "Aghlabid Emirate", "Makkura", "Idrisid Caliphate", "Alwa", "Kanem",
    "Axum", "Ghana",
    "Huari Empire", "Tiahuanaco Empire", "Toltec Empire",
    "Abbasid Caliphate", "Tibetan Empire", "Gurjara Pratihara",
    "Rashtrakuta state", "Palas", "Chauhans", "Pallava state",
    "Pandya state", "Vallabhi",
    "Ouighurs", "Tang Empire", "Srivijaya Empire", "Nan Chao", "Japan",
    "Parhae", "Chen-La", "Silia", "Mataram", "Pyu state", "Champa",
    "Mon state", "Dvaravati",
}

NAME_JA = {
    "Byzantine Empire": "ビザンツ帝国(東ローマ帝国)",
    "Carolingian Empire": "カロリング帝国(フランク王国)",
    "Khazars": "ハザール・カガン国", "Volga Bulgars": "ヴォルガ・ブルガール",
    "Danes": "デンマーク(デーン人)", "Swedes and Goths": "スウェーデン(スヴェア人)",
    "Bulgars": "ブルガリア帝国(第一次)", "Asturias": "アストゥリアス王国",
    "Emirate of Córdoba": "コルドバ首長国", "Cyprus": "キプロス(ビザンツ領)",
    "Britany": "ブルターニュ公国", "Irlanda": "アイルランド(諸小王国)",
    "Welsh": "ウェールズ(諸小王国)", "Scots": "ダル・リアタ王国(スコット人)",
    "Picts": "ピクト王国", "Mercia": "マーシア王国", "Wessex": "ウェセックス王国",
    "Nothumbria": "ノーサンブリア", "Essex": "エセックス王国", "Kent": "ケント王国",
    "Lombard duchies": "南イタリア・ロンバルド諸公国", "Magyars": "マジャル人(ハンガリー建国前)",
    "Rus' Khaganate": "ルーシ・カガン国(建国期)", "Papal States": "教皇領",
    "Avars": "アヴァール可汗国",
    ...
    "Gurjara Pratihara": "グルジャラ・プラティーハーラ朝",
    "Rashtrakuta state": "ラーシュトラクータ朝", "Palas": "パーラ朝(ベンガル)",
    "Chauhans": "チャハマーナ朝(チャウハーン朝)", "Pallava state": "パッラヴァ朝",
    "Pandya state": "パーンディヤ朝", "Vallabhi": "ヴァラビー王国(マイトラカ朝)",
    "Ouighurs": "ウイグル可汗国", "Tang Empire": "唐",
    "Srivijaya Empire": "シュリーヴィジャヤ王国", "Nan Chao": "南詔",
    "Japan": "日本(平安時代)", "Parhae": "渤海", "Chen-La": "チェンラ王国(真臘)",
    "Silia": "新羅", "Mataram": "マタラム王国(ジャワ)",
    "Pyu state": "ピュー諸都市国家(ビルマ)", "Champa": "チャンパ王国",
    "Mon state": "モン族の国(ビルマ)", "Dvaravati": "ドヴァーラヴァティー王国",
}
...
REGIONS = {
    "europe": [
        "Byzantine Empire", "Carolingian Empire", "Khazars", "Volga Bulgars",
        "Danes", "Swedes and Goths", "Bulgars", "Asturias",
        "Emirate of Córdoba", "Cyprus", "Britany", "Irlanda", "Welsh",
        "Scots", "Picts", "Mercia", "Wessex", "Nothumbria", "Essex", "Kent",
        "Lombard duchies", "Magyars", "Rus' Khaganate", "Papal States",
        "Avars",
    ],
    "africa": [
        "Aghlabid Emirate", "Makkura", "Idrisid Caliphate", "Alwa", "Kanem",
...
LABEL_JA_SHORT = {
    "Carolingian Empire": "カロリング帝国", "Swedes and Goths": "スウェーデン",
    "Bulgars": "ブルガリア帝国", "Emirate of Córdoba": "コルドバ首長国",
    "Irlanda": "アイルランド", "Scots": "ダル・リアタ王国",
    "Nothumbria": "ノーサンブリア", "Lombard duchies": "ロンバルド諸公国",
    "Magyars": "マジャル人", "Rus' Khaganate": "ルーシ・カガン国",
    "Aghlabid Emirate": "アグラブ朝", "Idrisid Caliphate": "イドリース朝",
    "Makkura": "マクリア王国", "Tiahuanaco Empire": "ティワナク帝国",
    "Abbasid Caliphate": "アッバース朝", "Tibetan Empire": "吐蕃",
    "Gurjara Pratihara": "プラティーハーラ朝", "Rashtrakuta state": "ラーシュトラクータ朝",
    "Palas": "パーラ朝", "Chauhans": "チャウハーン朝", "Pallava state": "パッラヴァ朝",
    "Vallabhi": "ヴァラビー王国", "Ouighurs": "ウイグル可汗国",
    "Srivijaya Empire": "シュリーヴィジャヤ", "Mataram": "マタラム王国",
    "Pyu state": "ピュー諸都市国家", "Mon state": "モン族の国",
    "Dvaravati": "ドヴァーラヴァティー",
}
...
def main():
    src = json.load(open(os.path.join(BASE, "world_800.geojson"), encoding="utf-8"))
    feats = src["features"]
    ...
    # 統合ページ(world-history-puzzle)の data/800.json に直接出力する
    out_dir = os.path.join(BASE, "..", "..", "data")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "800.json")
